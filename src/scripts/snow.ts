...
export function initSnow() {
  ...
  let clearAnim: ClearAnimation | null = null;
  ...
  function getSnowSurfaceAtX(x: number) {
  ...
  function accumulateAt(x: number, amount: number) {
  ...
  function settleFlake(flake: SnowFlake) {
    if (clearAnim) {
  ...
  function drawSnowBank() {
    ...
    const getVisibleHeight = (value: number) => (value > 0 ? Math.max(1, value) : 0);
  ...
  function updateClearAnimation(now: number) {
    if (!clearAnim) return;
