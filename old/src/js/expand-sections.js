...
import { ANIMATION_DURATION, ANIMATION_EASING, SELECTORS } from './constants.js';

/**
 * Initialize expandable sections with smooth animations
 */
export function initExpandSectionAnimations() {
  const sections = Array.from(document.querySelectorAll(SELECTORS.expandSections));
  ...
  const clearAnimationStyles = body => {
    body.style.height = '';
    body.style.opacity = '';
    body.style.overflow = '';
  ...
  const cancelRunningAnimation = section => {
  ...
  const finishAnimation = (section, body, shouldStayOpen) => {
    clearAnimationStyles(body);
    section.open = shouldStayOpen;
    activeAnimations.delete(section);
  };

  const animateOpen = (section, body) => {
  ...
  const animateClose = (section, body) => {
    cancelRunningAnimation(section);
    section.open = true;
  ...
  sections.forEach(section => {
    const summary = section.querySelector(SELECTORS.expandSummary);
    const body = section.querySelector(SELECTORS.expandBody);
