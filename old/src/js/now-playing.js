...
let npLastStateKey = '';
let npCountdownTimerId;
let npFullscreenIdleTimerId;

/**
 * Create and inject CSS keyframes for scroll animation with pause timing
 */
function createScrollAnimationWithPauses(totalDuration) {
 ...
 */
function formatNowPlayingStatus({ isPlaying, statusText }) {
 ...
 */
function extractImageColor(imgEl) {
...
function setNowPlayingLoading(isLoading) {
  ...
  if (isLoading) {
    ...
    if (playCountLinkEl) {
      playCountLinkEl.removeAttribute('href');
...
function setNowPlayingError() {
  ...
  if (artLinkEl) {
    artLinkEl.removeAttribute('href');
 ...
 */
function applyNowPlayingScroll(textEl, lineEl) {
...
async function fetchRecentTrack(retryCount = 0) {
  ...
  try {
    ...
    if (albumEl) {
      ...
      albumEl.removeAttribute('href'); // Removed url
      ...
      if (artistPlayCount > 0 && artist) {
        ...
        if (artistMetaNameEl) {
          ...
          if (artistUrl) {
            ...
            artistMetaNameEl.removeAttribute('href');
          }
        }
        ...
        if (artistMetaCountEl) {
          ...
          if (artistLibraryUrl) {
            ...
            artistMetaCountEl.removeAttribute('href');
          }
        }
      } else {
        artistMetaEl.hidden = true;
    ...
    if (artLinkEl) {
      artLinkEl.removeAttribute('href'); // Removed url
...
export function initNowPlaying() {
  ...
  const openViewer = (e) => {
    ...
    if (ivPlayCount && ivPlayCountLink) {
      if (state.albumPlayCount > 0) {
        ...
        if (state.albumLibraryUrl) {
          ...
          ivPlayCountLink.removeAttribute('href');
        }
    ...
    if (ivLink) {
      ivLink.href = state.albumUrl || state.trackUrl || state.artistUrl || '#';
    }
